...
- Research question: __In the Natural Scenes Dataset (NSD) subject 1, session 1, is the mean beta response different in V1 and hV4?__
- Optional extension: __If the core pipeline works, repeat the same Region of Interest (ROI) comparison for a few additional sessions or revisit the V1 session-drift question.__
...
*Posterior view of an NSD visual ROI mask: grey points are valid brain voxels (volumetric picture element) outside this visual ROI set; colored points show V1, V2, V3, and hV4 subdivisions. The practical task is to connect voxel coordinates, ROI labels, trials, and beta values into one small analysis table.*
...
The practical uses the [**Natural Scenes Dataset (NSD)**](https://naturalscenesdataset.org/) containing fMRI data. European fMRI datasets
...
be shared through AWS Open Data after signing the [NSD data access agreement](https://docs.google.com/forms/d/e/1FAIpQLSduTPeZo54uEMKD-ihXmRhx0hBDdLHNsVyeo_kCb8qbyAkXuQ/viewform).


- Dataset and documentation: [naturalscenesdataset.org](https://naturalscenesdataset.org/)
- Main reference paper: Allen et al. (2022), Nature Neuroscience. doi: [https://doi.org/10.1038/s41593-021-00962-x](https://doi.org/10.1038/s41593-021-00962-x)
- Optional extension reference for session drift: Roth and Merriam (2023), Nature Communications. doi: [https://doi.org/10.1038/s41467-023-40144-w](https://doi.org/10.1038/s41467-023-40144-w)

Minimum NSD files needed for this project:
...
information. They are not arbitrary labels: they define which voxels
belong to each early visual-processing areas.
...
- BIDS documentation for neuroimaging data organization and metadata (  `nsddata_rawdata/dataset_description.json`,
  `nsddata_rawdata/participants.tsv`, and
  `nsddata_rawdata/task-nsdcore_bold.json`.).
- Basic introductions to Neuroimaging Informatics Technology Initiative (NIfTI) files, JSON sidecars, events files, and participant
  metadata.
- NSD documentation, the [main paper](https://doi.org/10.1038/s41593-021-00962-x), and the dataset manual.
...
Start from the raw scientific repository, identify the files that you need, and make a written manifest before downloading large files.

Week 1 data checklist:
...
- Explain why scientific data standards exist, referring your explanation to the [FAIR principles](https://doi.org/10.1038/sdata.2016.18).
- Explain the difference and complementary aspects between an event TSV, a beta NIfTI file, and an ROI
  mask.
...
Data cleaning: build the smallest useful analysis table from the raw files.
...
- Explain why trial-level values are not the same as raw Blood Oxygen-Level Dependent (BOLD) time series.
...
> Scientific neuroimaging programming is not just fitting a model. It is knowing how raw
